...
class Queue extends Emitter {
  ...
  _evalScriptOn(commandable, args) {
    ...
    return helpers
      .callAsync((done) => {
        args[0] = lua.shas[scriptName];
        args.push(done);
        commandable.evalsha.apply(commandable, args);
      })
      .catch((error) => {
        // Callback for error caused by a missing Lua script
        if (error.message === 'NOSCRIPT No matching script. Please use EVAL.') {
          return helpers.callAsync((done) => {
            args[0] = lua.scripts[scriptName];
            // Replace done callback
            args.pop();
            args.push(done);
            commandable.eval.apply(commandable, args);
          });
        } else {
          throw error;
        }
